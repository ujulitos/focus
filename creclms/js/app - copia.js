                            ...
                            childSnapshot.forEach(function(childSnapshot2) {
                                ...
                                childSnapshot2.forEach(function(childSnapshot3) {
                                    ...
                                    if (childSnapshot3.key != "Clase") {
                                        ...
                                        this['CursoKey' + contadorRecursosDisponibles] = childSnapshot3.key;
                                        console.log('CursoKey' + contadorRecursosDisponibles + ': ', this['CursoKey' + contadorRecursosDisponibles]);
...
function leeEstatusCurso(estatusNo) {
    if (revisaConexion) {
        ...
        var elRefRevisado = laUrlBase + 'Lecciones/' + usuarioId + '/objeto_' + Key_oc + elSufijo;
        ...
        // SCORM //
        elRefSCORM = laUrlBase + 'Lecciones/' + usuarioId + '/objeto_' + Key_oc + elSufijo + '/SCORM_12';
    ...
    for (c = 1; c <= cuantosRecursosVisibles; c++) {
        ...
        for (d = 1; d <= cuantosRecursosDisponibles; d++) {
            ...
            if (those['elCursoVisible' + d] == Id_oc) {
                ...
                Key_oc = those['CursoKey' + d];
                console.log('Key_oc', d, Key_oc);
...
function activaLanzarCursos() {
    ...
    for (a = 1; a <= cuantosRecursos; a++) {
        ...
        $("#botonIniciarCurso" + a).click(function(event) {
            ...
            lanzaContenido(elContenido, Key_oc);
        });
    }
}
...
function lanzaContenido(cualContenido, cualKey) {
    elContenido = cualContenido;
    console.log('cualContenido', elContenido);
    console.log('CursoId', that['CursoId' + elContenido]);
    console.log('CursoKey', cualKey);
    elSufijo = that['CursoId' + elContenido].substr(6, 2);
    console.log('elSufijo', elSufijo);
...
function registraIniciado() {
    ...
    if (revisaConexion) {
        ...
        var elRefIniciado2;
        var elCursoPre;
        var elSufijoDX = Key_oc + 'dx';
        // var elSufijoOC = 'oc';
        var elSufijoOC = Key_oc + 'oc';
        // elContenido = $(this).attr('id').substr(17, 3);
        console.log('CursoKey', Key_oc);
...
function registraRevisado() {
    ...
    if (revisaConexion) {
        ...
        var elRefRevisado = laUrlBase + 'Lecciones/' + usuarioId + '/' + elCurso + 'objeto_' + Key_oc + elSufijo;
        console.log('elRefRevisado: ', elRefRevisado);
...
function registraCalificacion(cualCalificacion) {
    ...
    if (revisaConexion) {
        ...
        var elRefCalificacion = laUrlBase + 'Lecciones/' + usuarioId + '/' + elCurso + '/objeto_' + Key_oc + elSufijo;
...
$(document).ready(function() {
    ...
    $("#boton_inicio, #boton_recursos, #boton_usuarios, #boton_lecciones, #boton_admin, #boton_resultados").click(function(event) {
