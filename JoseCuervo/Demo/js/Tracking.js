...
function initCourse() {
    ...
    $("#btnPrev, #btnNext").mouseover(function() {
        if ($(this).hasClass('btnNavOver')) {
            if (!bloqueoGral && !bloqueoRegreso) {
                $(this).css({
                    // 'filter': 'brightness(1) sepia(0.1) saturate(100%)'
                })
            }
        }
    });
...
function setScale() {
    ...
    if (pantallaCompleta) {
        ...
        // $('#progressBar').css("bottom", "" + (22 * scaleTemplate) + "px");
        $('#progressBar').css("left", "" + (600 * scaleTemplate) + "px");
        $('#progressBar').css("bottom", "" + (5 * scaleTemplate) + "%");

        // $('#progressDatos').css("top", "" + (15 * scaleTemplate) + "px");
        $('#progressDatos').css("left", "" + (105 * scaleTemplate) + "px");
