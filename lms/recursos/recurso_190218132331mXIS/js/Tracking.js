        ...
        if (elSuspendData == null || elSuspendData == undefined || elSuspendData == '' || elSuspendData == 'null') {
            ...
            if (conIntentos) {
                if (losIntentosNum >= maxIntentos) {
                    if (currentPage == course.pages.length - 2) {
                        // if (intentos == 0) {
                        currentPage--;
                        $("#modalBloq").fadeIn();
                        // return false;
...
function updateTracking() {
    ...
    if (conection) {
        ...
        console.log('hayEvaluacion', hayEvaluacion);
        if (!curseCompleted) {
            if (!hayEvaluacion) {
                if (currentPage == course.pages.length - 2) {
                ...
                if (currentPage == course.pages.length - 2 && hayCalificacion >= minimoAprobatorio) {
                    // set("cmi.core.lesson_status", "completed");
                    // var success = save();
                    curseCompleted = true;
                    registraCompleto();
