	...
	function loadXML(fileName, randomQuestions, randomAnswers){
		//console.log("ready to go...");
		var file = "./opcion_multiple.xml";
		if (fileName!="") {
			file=fileName;
		}// if fileName!=""
		corrects=0;
				...
				//console.log("q.length: " + q.length);
				  ...
				  //	console.log("a.length: " + a.length);
				    ...
				    if (randomAnswers){ // respuestas aleatorias
				   	...
				   	questions.push(new question(q[i].getAttribute("text"), q[i].getAttribute("number"), aArray));
				   }//for i questions
	...
	function setUserAnswer(numAnswer){
	...
	function isCorrect(numAnswer){
		//console.log("numAnswer: " + numAnswer + " - current - " + current);
	...
	function question(text, number, answers) {
